...
;       PNG      : Creates PNG file instead of Xwindow.  If set to a string, 
;                  this will be the name of the gif, if set to 1, the png
;                  will be named idl.png. 
...
sc_spin_axis_sm,sc_pos_geo,geo_N_sm,nadir,GIF=GIF, PNG=PNG,WSIZE=WSIZE,$
...
;  red[254]=255 & green[254]=255 & blue[254]=255 & mywhite=254 ; make color=254 white
  ...
  ; deviceopen,6,fileOutput=GIF,sizeWindow=[x_wsize,y_wsize]
   ...
   s = size(PNG)
  if (s[n_elements(s)-2] ne 7) then PNG='idl.png'
  set_plot,'z'
  tvlct,red,green,blue,/get
;  loadct, 13
...
if keyword_set(PNG) then begin
  xscale=!x.s & yscale=!y.s & bytemap=tvrd() & tvlct,r,g,b,/get
  s = size(PNG) & ns = n_elements(s)
  if s[ns-2] eq 7 then gname = PNG else gname = 'idl.png'
  write_png,gname,bytemap,r,g,b
  device,/close & set_plot,'X'
endif
...
PRO testplot_enaflux3,imgfile,GIF=GIF,PNG=PNG,WSIZE=WSIZE,NOCIRCLES=NOCIRCLES,$
...
s=plot_enaflux3(etime,image,spin_angle,polar_angle,np,sc_pos_sm,$
                ...
                GIF=GIF,PNG=PNG,WSIZE=WSIZE,NOCIRCLES=NOCIRCLES,NODIPOLES=NODIPOLES,$
...
FUNCTION plot_enaflux,etime,image,fov,resolution,sc_pos_gci,sc_spinaxis_gci,$
                      nadir,REFORMER=REFORMER,ORBIT=ORBIT,GIF=GIF,PNG=PNG,WSIZE=WSIZE,$
                      NOCIRCLES=NOCIRCLES,NODIPOLES=NODIPOLES,NOEARTH=NOEARTH,$
                      NOCOLORBAR=NOCOLORBAR,EDGES=EDGES,NOBORDER=NOBORDER,$
...
; Generate the plot
s = plot_enaflux3(etime,image,spin_angle,polar_angle,np,sc_pos_sm,$
                            sc_spinaxis_sm,sc_pos_geo,geo_N_sm,nadir,GIF=GIF,PNG=PNG,$
