...
;$Header: /home/cdaweb/dev/control/RCS/combined_subtitle.pro,v 1.23 2022/04/19 20:49:46 tkovalic Exp tkovalic $
;$Locker: tkovalic $
;$Revision: 31135 $
;+------------------------------------------------------------------------
...
;TJK 4/19/2022 uncomment ps lines - Bobby/Lan want the subtitles for
;                                   PS and PDF files
...
;TJK 4/19/2022 use length of pi string vs. title to determin width of characters
  if ((!d.x_ch_size * strlen(pi)) gt !d.x_size) then begin
...
save_c = c
number_of_lines = number_of_lines + 1 ; need to account for the date stamp line

; Write the subtitle into the window

;TJK 4/20/2022 greatly simplified the code to use a "factor" to
;determine the offset from the bottom of the plotting window. Depending
;on the "device" being used, its slightly different.
  if (c ge 0.75) then begin
    ;TJK make the font smaller for PS output
    if keyword_set(ps) then begin ;theoretically this logic should handle all ps cases (number of lines)
       c = 0.70
       factor = !d.y_ch_size*(3.7); seems to work for PS
    endif else begin
       factor = !d.y_ch_size*(4.8); seems to work for PNG/GIF/X
   endelse
       xyouts,!d.x_size/2,factor,pi_final,/DEVICE,ALIGNMENT=0.5,CHARSIZE=c
       xyouts,!d.x_size/2,!d.y_ch_size/4L,s,/DEVICE,ALIGNMENT=0.5,CHARSIZE=c
    if (ps) then c = save_c
 endif
  ...
  if (c lt 0.75) then begin ;TJK added to handle label when
    ...
    gen_date = ' Generated by '+ web_code + ' on ' + systime()
      ...
      6: begin
         xyouts,!d.x_size/2,50L,pi_final,/DEVICE,ALIGNMENT=0.5,CHARSIZE=c
